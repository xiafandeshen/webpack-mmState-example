...
define(["avalon", "mmPromise"], function(avalon) {
    ...
    avalon.component("oni:validation", {
      ...
      data: [], //@config {Array} [] 储放验证数据
      form: {}, //@config {Element} {} 表单元素
      $skipArray: ["data", "validationHooks", "validateInKeyup",
        "validateAllInSubmit", "resetInBlur", "form"
      ],
      /**
      ...
      $init: function(vm, element) {
        ...
        if (form && form.tagName === "FORM") {
          ...
          if (vm.validateAllInSubmit) {
            onSubmitCallback = avalon.bind(form, "submit", function(e) {
              e.preventDefault()
        ...
        vm.$watch("avalon-ms-duplex-init", function(data) {
          ...
          data.valueAccessor = function(a) {
            if (arguments.length) {
              ...
              return data.getter.apply(0, data.vmodels)
            }
          }
      ...
      $dispose: function(vm, element) {
        vm.data = []
        onSubmitCallback && avalon.unbind(vm.form, "submit", onSubmitCallback)
      ...
      $ready: function(vm, element) {
        vm.validateAll = function(callback) {
          ...
          var promise = vm.data.filter(function(data) {
            var el = data.element
            return el && !el.disabled && element.contains(el)
          }).map(function(data) {
          ...
          Promise.all(promise).then(function(array) {
            ...
            fn.call(element, reasons) //这里只放置未通过验证的组件
          })
        }
                ...
                var next = function(a) {
                  ...
                  if (a) {
                    ...
                    var reason = {
                      element: elem,
                      data: data.data,
                      message: elem.getAttribute("data-duplex-" + name + "-message") ||
                        elem.getAttribute("data-duplex-message") || hook.message,
                      validateRule: name,
